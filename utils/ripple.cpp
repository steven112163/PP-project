...
#pragma omp parallel for collapse(2)
    ...
    z = 0;
    for (x = 1; x < surface_size - 1; x++) {
        point = glm::vec3(surface->get_vertex(surface_stride * z + 3 * x, state),
                          surface->get_vertex(surface_stride * z + 3 * x + 1, state),
                          surface->get_vertex(surface_stride * z + 3 * x + 2, state));

        pos_z = glm::vec3(surface->get_vertex(surface_stride * (z + 1) + 3 * x, state),
                          surface->get_vertex(surface_stride * (z + 1) + 3 * x + 1, state),
                          surface->get_vertex(surface_stride * (z + 1) + 3 * x + 2, state)) - point;
        neg_x = glm::vec3(surface->get_vertex(surface_stride * z + 3 * (x - 1), state),
                          surface->get_vertex(surface_stride * z + 3 * (x - 1) + 1, state),
                          surface->get_vertex(surface_stride * z + 3 * (x - 1) + 2, state)) - point;
        pos_x = glm::vec3(surface->get_vertex(surface_stride * z + 3 * (x + 1), state),
                          surface->get_vertex(surface_stride * z + 3 * (x + 1) + 1, state),
                          surface->get_vertex(surface_stride * z + 3 * (x + 1) + 2, state)) - point;
        new_normal = glm::normalize(glm::cross(neg_x, pos_z) +
                                    glm::cross(pos_z, pos_x));
    ...
    for (z = 1; z < surface_size - 1; z++) {
        ...
        neg_z = glm::vec3(surface->get_vertex(surface_stride * (z - 1) + 3 * x, state),
                          surface->get_vertex(surface_stride * (z - 1) + 3 * x + 1, state),
                          surface->get_vertex(surface_stride * (z - 1) + 3 * x + 2, state)) - point;
        pos_z = glm::vec3(surface->get_vertex(surface_stride * (z + 1) + 3 * x, state),
                          surface->get_vertex(surface_stride * (z + 1) + 3 * x + 1, state),
